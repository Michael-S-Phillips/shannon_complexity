...
#     """
#     def __init__(self, img_path, tile_size, overlap, kernel_sizes, out_dir=None, crop=False):
...
#             futures = [executor.submit(self.kernel_entropy, window) for window in windows]
...
#                     window = window.intersection(Window(0, 0, self.img_width, self.img_height))
...
#         for b, kernel_size in tqdm(enumerate(self.kernel_sizes)):
#             # Initialize an array to store the complexity values
#             kernel_complexity_map = np.zeros_like(base_img)
#             padded_map = np.pad(base_img, pad_width=kernel_size // 2, mode='constant', constant_values=0)
...
#                 # complexity_map_cube = complexity_map_cube[min(0, (window.row_off - self.tile_size //2)):max(self.img_height, (window.row_off + self.tile_size//2)),
#                 #                                           min(0, (window.col_off - self.tile_size //2)):max(self.img_width, (window.col_off + self.tile_size//2))]
#                 complexity_map_cube = complexity_map_cube[(window.row_off + self.overlap//2):(window.row_off + (self.tile_size - self.overlap//2)),
#                                                           (window.col_off + self.overlap//2):(window.col_off + (self.tile_size - self.overlap//2))]
#                 window_transform = rasterio.windows.transform(
#                     Window(
#                         window.col_off + self.overlap//2,
#                         window.row_off + self.overlap//2,
#                         self.tile_size,
#                         self.tile_size
#                     ),
#                     src_transform
